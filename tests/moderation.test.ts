...
describe('moderación local aislada por asistente', () => {
  ...
  it('migra desactivada, sin IA, expulsión ni eliminación automática', () => {
    expect(database.getModerationSettings('neurobot')).toMatchObject({
      ...
      temporaryEvidenceEnabled: true,
      temporaryEvidenceHours: 72,
    });
  ...
  it('mantiene reglas independientes por assistantId', () => {
    ...
    database.createModerationRule(
      'neurobot',
      ruleInput('Regla Neurobot', 'EXACT_WORD', 'prohibida', 4),
    );
  ...
  it('respeta configuración independiente por grupo', () => {
    ...
    expect(database.listModerationGroupSettings('neurobot')).toEqual([
      { groupHash: 'a'.repeat(20), mode: 'DISABLED', enabled: false },
      { groupHash: 'b'.repeat(20), mode: 'ENABLED', enabled: true },
    ]);
  });

  it('detecta palabra completa sin coincidencias parciales', () => {
    const engine = new LocalModerationEngine();
    const settings = database.getModerationSettings('neurobot');
    const rule = storedRule(database, ruleInput('Palabra', 'EXACT_WORD', 'sol', 4));
    expect(engine.evaluate(simulation('tomamos sol'), settings, [rule], []).action).toBe(
      'WARNING_AND_NOTIFY',
    );
  ...
  it('detecta frases y palabras combinadas', () => {
    ...
    const combined = storedRule(
      database,
      ruleInput('Combinada', 'COMBINED_WORDS', 'venta|privada', 3),
    );
    expect(
      engine.evaluate(simulation('esta frase prohibida aparece'), settings, [phrase], []).action,
    ).toBe('ADMIN_REVIEW');
    expect(
      engine.evaluate(simulation('venta autorizada pero privada'), settings, [combined], []).action,
    ).toBe('ADMIN_REVIEW');
  });
  ...
  it('aplica excepciones antes de puntuar', () => {
    ...
    input.exceptions = [
      { id: 0, exceptionType: 'ALLOWED_DOMAIN', normalizedValue: 'ejemplo.test', enabled: true },
    ];
    const rule = storedRule(database, input);
    const result = engine.evaluate(simulation('visita https://ejemplo.test'), settings, [rule], []);
    expect(result).toMatchObject({ action: 'NO_ACTION', totalScore: 0 });
  ...
  it('detecta datos personales conservadoramente y evita mayúsculas breves', () => {
    ...
    expect(
      engine.evaluate(simulation('mi correo es persona@ejemplo.test'), settings, [personal], [])
        .action,
    ).toBe('WARNING_AND_NOTIFY');
  ...
  it('detecta repetición local sin guardar mensajes', () => {
    const engine = new LocalModerationEngine();
    const settings = database.getModerationSettings('neurobot');
    const input = ruleInput('Spam', 'REPETITION', '', 4);
    input.conditions[0]!.configuration = { count: 3, windowSeconds: 120 };
    const rule = storedRule(database, input);
    expect(
      engine.evaluate(
        { ...simulation('repetido'), messageHash: '1', simulate: false },
        settings,
        [rule],
        [],
      ).action,
    ...
    expect(
      engine.evaluate(
        { ...simulation('repetido'), messageHash: '2', simulate: false },
        settings,
        [rule],
        [],
      ).action,
    ...
    expect(
      engine.evaluate(
        { ...simulation('repetido'), messageHash: '3', simulate: false },
        settings,
        [rule],
        [],
      ).action,
...
describe('incidentes y advertencias locales', () => {
  ...
  beforeEach(() => {
    ...
    const rulesText = 'No se permite contenido con la palabra prohibida dentro del grupo.';
    const rulesHash = 'hash-reglas';
    database.saveGroupModerationDraft('neurobot', 'grupo', rulesText, rulesHash);
    database.saveCompiledGroupModeration({
        ...
        rules: [
          {
            ...
            score: 4,
            conditions: [
              { type: 'EXACT_WORD', value: 'prohibida', operator: 'ANY', configuration: {} },
            ],
            exceptions: [],
          },
        ],
        tests: [
          { text: 'mensaje amable', expected: 'ALLOW', category: 'RESPETO' },
          { text: 'prohibida', expected: 'WARNING', category: 'RESPETO' },
        ],
        summary: { categories: ['Respeto'], protectedBehaviors: ['Convivencia'], exceptions: [] },
      },
      summary: { categories: ['Respeto'], protectedBehaviors: ['Convivencia'], exceptions: [] },
    });
    for (const test of [
      { type: 'AUTOMATIC' as const, expected: 'ALLOW' as const },
      { type: 'AUTOMATIC' as const, expected: 'WARNING' as const },
      { type: 'MANUAL_ALLOWED' as const, expected: 'ALLOW' as const },
      { type: 'MANUAL_WARNING' as const, expected: 'WARNING' as const },
    ])
      database.recordGroupModerationTest({
        assistantId: 'neurobot',
        groupHash: 'grupo',
        rulesHash,
        testType: test.type,
  ...
  it('no analiza cuando se desactiva y funciona sin Groq', async () => {
    ...
    const result = await service.process(
      message('m1', 'contenido prohibida'),
      'grupo',
      'persona',
      'm1',
    );
  ...
  it('ignora mensajes fromMe y analiza mensajes nuevos de otra cuenta', async () => {
    expect(
      (
        await service.process(
          { ...message('propio', 'contenido prohibida'), fromMe: true },
          'grupo',
          'persona',
          'propio',
        )
    ...
    expect(
      (
        await service.process(
          message('externo', 'contenido prohibida'),
          'grupo',
          'persona',
          'externo',
        )
  ...
  it('envía una advertencia neutral, crea caso y consume cero tokens', async () => {
    const result = await service.process(
      message('m2', 'contenido prohibida'),
      'grupo',
      'persona',
      'm2',
    );
  ...
  it('suprime el mismo mensaje y nunca expulsa ni elimina', async () => {
    ...
    const duplicate = await service.process(
      message('m3', 'contenido prohibida'),
      'grupo',
      'persona',
      'm3',
    );
  ...
  it('una reincidencia crea segunda advertencia y aviso administrativo', async () => {
    ...
    database.saveModerationRecurrence(
      'neurobot',
      'grupo',
      'persona',
      1,
    ...
    expect(
      client.sentMessages.some((sent) => sent.text.includes('Segunda advertencia automática')),
    ).toBe(true);
    ...
    expect(
      database
        .getTechnicalEvents()
        .some((event) => event.event_type === 'MODERATION_ADMIN_NOTIFIED'),
    ).toBe(true);
  });
  ...
  it('falso positivo corrige la reincidencia y evidencia expira', async () => {
    ...
    expect(
      database.reviewModerationCase('neurobot', Number(moderationCase.id), 'FALSE_POSITIVE'),
    ).toBe(true);
    ...
    database.createModerationCase({
      assistantId: 'neurobot',
      groupHash: 'grupo',
      participantHash: 'otra',
      messageHash: 'expirada',
      category: 'OTRA',
      matchedRuleIds: [],
      score: 3,
      severity: 'MEDIA',
      ...
      encryptedEvidence: 'v1.cifrada',
      evidenceExpiresAt: new Date(Date.now() - 1000).toISOString(),
    });
    expect(database.expireModerationEvidence('neurobot')).toBe(1);
  });

  it('el probador no guarda texto, no crea casos ni envía mensajes', () => {
    const before = database.listModerationCases('neurobot').length;
    const result = service.test('contenido prohibida');
    expect(result.action).toBe('WARNING_AND_NOTIFY');
    expect(database.listModerationCases('neurobot')).toHaveLength(before);
    expect(client.sentMessages).toHaveLength(0);
...
function ruleInput(
  ...
  value: string,
  score: number,
): Omit<ModerationRule, 'id' | 'assistantId' | 'createdAt' | 'updatedAt'> {
  return {
    name,
    description: 'Regla estructurada de prueba',
    category: 'RESPETO',
    severity: score >= 4 ? 'ALTA' : 'MEDIA',
    detectionType: conditionType,
    score,
    reviewThreshold: 3,
    warningThreshold: 4,
...
function storedRule(database: AppDatabase, input: ReturnType<typeof ruleInput>) {
  return database.createModerationRule('neurobot', input);
}
function simulation(text: string) {
  return {
    assistantId: 'neurobot',
    groupHash: 'grupo',
    participantHash: 'persona',
    messageHash: `m-${Math.random()}`,
    text,
    isAdministrator: false,
    simulate: true,
  };
}
function message(id: string, body: string): IncomingMessage {
  return {
    id,
    chatId: 'grupo-real@g.us',
