...
describe('aislamiento multibot', () => {
  ...
  it('crea dos bots con perfiles, sesiones, menús y datos independientes', () => {
    const first = database.createBot({
      id: 'tienda-uno',
      mode: 'business',
      sessionPath: 'data/sessions/uno',
      profile: storeProfile('Uno'),
    });
    const second = database.createBot({
      id: 'tienda-dos',
      mode: 'mixed',
      sessionPath: 'data/sessions/dos',
  ...
  it('selecciona opciones por número, nombre y alias', () => {
    const bot = database.createBot({
      id: 'tienda-menu',
      mode: 'business',
      sessionPath: 'data/sessions/menu',
  ...
  it('usa fallback numerado y mantiene un estado temporal sin conversación completa', async () => {
    const bot = database.createBot({
      id: 'tienda-flujo',
      mode: 'business',
      sessionPath: 'data/sessions/flujo',
  ...
  it('no inventa precios ausentes', () => {
    const bot = database.createBot({
      id: 'tienda-precio',
      mode: 'business',
      sessionPath: 'data/sessions/precio',
  ...
  it('mantiene automatizaciones y encuestas separadas por bot', () => {
    const first = database.createBot({
      id: 'tienda-auto-uno',
      mode: 'business',
      sessionPath: 'data/sessions/auto-uno',
      profile: storeProfile('Auto Uno'),
    });
    const second = database.createBot({
      id: 'tienda-auto-dos',
      mode: 'business',
      sessionPath: 'data/sessions/auto-dos',
  ...
  it('no bloquea bots por un presupuesto global interno', () => {
    const first = database.createBot({
      id: 'tienda-ia-uno',
      mode: 'business',
      sessionPath: 'data/sessions/ia-uno',
      profile: storeProfile('IA Uno'),
    });
    const second = database.createBot({
      id: 'tienda-ia-dos',
      mode: 'business',
      sessionPath: 'data/sessions/ia-dos',
    ...
    expect(reservation(second.id, second.profileId, 'dos')).toMatchObject({ allowed: true });
