...
describe('plataforma de asistentes', () => {
  ...
  it('construye módulos distintos para comunidad, negocio y modo mixto', () => {
    ...
    const mixed = database.createBot({
      ...
      sessionPath: 'data/sessions/mixto-prueba',
      profile: businessProfile(),
    });
  ...
  it('rechaza una identidad duplicada y conserva intacto el asistente original', () => {
    ...
    const draft = database.createBot({
      id: 'borrador-duplicado',
      mode: 'mixed',
      connectorType: 'WHATSAPP_WEB',
      sessionPath: 'data/sessions/borrador-duplicado',
      profile: businessProfile(),
    });
    ...
    expect(database.getBot(draft.id)).toMatchObject({
      lifecycleStatus: 'DUPLICATE_CONFIGURATION',
      whatsappStatus: 'disconnected',
      maskedNumber: null,
    });
  });

  it('permite enviar, restaurar y conservar asistentes sin protección especial', () => {
    const first = database.createBot({
      id: 'borrador-papelera',
      mode: 'business',
      sessionPath: 'data/sessions/borrador-papelera',
      profile: businessProfile(),
    });
    const untouched = database.createBot({
      id: 'otro-asistente',
      mode: 'business',
      sessionPath: 'data/sessions/otro-asistente',
      profile: businessProfile(),
    });
    ...
    expect(database.restoreBotFromTrash(first.id, 'actor-hash')).toMatchObject({
  ...
  it('transfiere datos comerciales sin copiar la identidad ni la sesión', () => {
    const source = database.createBot({
      ...
      mode: 'business',
      sessionPath: 'data/sessions/borrador-transferible',
      profile: businessProfile(),
    });
    ...
    expect(database.getBot('neurobot')).toMatchObject({
      mode: 'mixed',
      operatingMode: 'BUSINESS_MIXED',
      privateBusinessModeEnabled: true,
      sessionPath: originalSession,
    });
    expect(database.listCatalogItems('neurobot')).toEqual(
      expect.arrayContaining([expect.objectContaining({ name: 'Enmarcado' })]),
    );
...
describe('navegación global y por asistente', () => {
  ...
  it('usa encabezado genérico y separa la navegación global', () => {
    ...
    expect(html).toContain('id="application-title">Neurobot AI</h1>');
    ...
    expect(html).toContain('data-section="status" class="bot-only hidden"');
  ...
  it('recarga módulos y datos con el assistantId seleccionado', () => {
    ...
    expect(panel).toContain("visible.has('automatic-messages')");
