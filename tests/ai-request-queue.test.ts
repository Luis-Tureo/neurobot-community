...
describe('cola de solicitudes de IA por asistente', () => {
  ...
  it('rechaza al superar la capacidad de espera sin mencionar tokens', async () => {
    ...
    await expect(
      queue.run({ flightKey: 'c', classifyError: classify, operation: async () => 'ok' }),
  ...
  it('agrupa preguntas iguales en una sola operación', async () => {
    ...
    await expect(Promise.all([first, second])).resolves.toMatchObject([
      { value: 'respuesta', coalesced: false },
      { value: 'respuesta', coalesced: true },
    ]);
  });
  ...
  it('reintenta errores temporales y no reintenta errores permanentes', async () => {
    ...
    await expect(
      queue.run({
        ...
        operation: async () => {
          permanentCalls += 1;
          throw new AIProviderError('AI_INVALID_KEY', 'permanente');
        },
      }),
  ...
  it('respeta Retry-After aunque supere el máximo usado para el backoff calculado', async () => {
    database.saveAIQueueSettings('neurobot', {
      ...database.getAIQueueSettings('neurobot'),
      maxRetries: 1,
      maximumRetryDelaySeconds: 10,
    ...
    const result = await queue.run({
      flightKey: 'retry-after-largo',
      classifyError: classify,
      operation: async () => {
        calls += 1;
        if (calls === 1) {
          throw new AIProviderError('AI_PROVIDER_RATE_LIMITED', 'temporal', true, 120);
        }
        return 'recuperado';
      },
    });
  ...
  it('aplica backoff sin Retry-After y agota los reintentos sin ciclos infinitos', async () => {
    ...
    await expect(
      queue.run({
        flightKey: 'limite-persistente',
        classifyError: classify,
        operation: async () => {
          calls += 1;
          throw new AIProviderError('AI_PROVIDER_RATE_LIMITED', 'temporal', true);
        },
      }),
  ...
  it('no reintenta antes de Retry-After cuando excedería la fecha límite del trabajo', async () => {
    database.saveAIQueueSettings('neurobot', {
      ...database.getAIQueueSettings('neurobot'),
      maxRetries: 2,
    });
    ...
    await expect(
      queue.run({
        flightKey: 'fecha-limite',
        classifyError: classify,
        deadlineAtMs: now + 5_000,
        operation: async () => {
          calls += 1;
          throw new AIProviderError('AI_PROVIDER_RATE_LIMITED', 'temporal', true, 10);
        },
      }),
    ).rejects.toMatchObject({ code: 'AI_PROVIDER_RATE_LIMITED' });
  ...
  it('registra solo diagnóstico seguro del límite sin mensajes ni credenciales', async () => {
    database.saveAIQueueSettings('neurobot', {
      ...database.getAIQueueSettings('neurobot'),
      maxRetries: 0,
    });
    ...
    const logger = {
      info: (context: unknown) => entries.push(context),
    } as unknown as Logger;
    const queue = new AIRequestQueueService(database, logger, 'neurobot');

    await expect(
      queue.run({
        flightKey: 'diagnostico-seguro',
        classifyError: classify,
        operation: async () => {
    ...
    expect(entries).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          operation: 'AI_PROVIDER_RATE_LIMITED',
  ...
  it('acepta consultas consecutivas sin pausa artificial', async () => {
    const queue = new AIRequestQueueService(database, createLogger('silent'), 'neurobot');
    await expect(
      queue.run({ flightKey: 'primera', classifyError: classify, operation: async () => 'uno' }),
    ).resolves.toMatchObject({ value: 'uno' });
    await expect(
      queue.run({ flightKey: 'segunda', classifyError: classify, operation: async () => 'dos' }),
  ...
  it('abre el circuito después de cinco fallos temporales', async () => {
    ...
    for (let index = 0; index < 5; index += 1) {
      await expect(
        queue.run({
          flightKey: `fallo-${index}`,
          classifyError: classify,
          operation: async () => {
            throw new AIProviderError('AI_TEMPORARY_ERROR', 'temporal', true);
          },
        }),
    ...
    await expect(
      queue.run({
        flightKey: 'bloqueada',
        classifyError: classify,
        operation: async () => 'no debe ejecutarse',
      }),
    ).rejects.toBeInstanceOf(AIQueueError);
  ...
  it('permite una prueba HALF_OPEN y cierra el circuito al recuperarse', async () => {
    ...
    await expect(
      queue.run({
        flightKey: 'prueba-recuperacion',
        classifyError: classify,
        operation: async () => 'recuperado',
      }),
    ).resolves.toMatchObject({ value: 'recuperado' });
    expect(queue.snapshot().providerHealth).toMatchObject({
      circuitState: 'CLOSED',
      state: 'AVAILABLE',
    });
...
describe('cola de salida por chat', () => {
  it('mantiene orden por chat sin mezclar colas', async () => {
    ...
    try {
      await Promise.all([
        outbound.send('grupo-a@g.us', 'primero'),
        outbound.send('grupo-a@g.us', 'segundo'),
        outbound.send('grupo-b@g.us', 'independiente'),
      ]);
      expect(
        client.sentMessages
          .filter((item) => item.chatId === 'grupo-a@g.us')
          .map((item) => item.text),
