...
describe('respuestas locales, caché y consumo real de IA', () => {
  ...
  it('convierte automáticamente una respuesta reutilizada en pregunta frecuente', async () => {
    ...
    addKnowledge(database, profileId, {
      title: 'Convivencia general',
      content: 'Las reuniones comunitarias se anuncian en el grupo oficial.',
      keywords: ['reuniones'],
    });
    const question = '¿Dónde se anuncian las reuniones comunitarias?';
    expect((await service.answerQuestion(question, 'group-a', 'user-a')).code).toBe('AI_RESPONSE');
    for (let index = 0; index < 3; index += 1) {
      expect((await service.answerQuestion(question, `group-${index}`, `user-${index}`)).code).toBe(
        'ANSWER_CACHE',
      );
    }
    expect((await service.answerQuestion(question, 'group-faq', 'user-faq')).code).toBe(
      'LOCAL_FAQ',
    );
    expect(provider.calls).toBe(1);
    database.close();
  ...
  it('no bloquea solicitudes por cuota de usuario', () => {
    const { database, profileId } = setup();
    ...
    expect(reserve(database, profileId, 'same-user', 'group-3')).toMatchObject({ allowed: true });
    database.close();
  });

  it('no bloquea solicitudes por cuota de grupo', () => {
    const { database, profileId } = setup();
    const settings = database.getAISettings(profileId);
    database.saveAISettings({
      ...settings,
    ...
    expect(reserve(database, profileId, 'user-3', 'same-group')).toMatchObject({ allowed: true });
    database.close();
  });

  it('no bloquea solicitudes por cuota diaria del bot', () => {
    const { database, profileId } = setup();
    const settings = database.getAISettings(profileId);
    database.saveAISettings({
      ...settings,
    ...
    expect(reserve(database, profileId, 'user-3', 'group-3')).toMatchObject({ allowed: true });
    database.close();
  });
