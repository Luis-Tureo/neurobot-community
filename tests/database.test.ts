...
describe('persistencia SQLite', () => {
  ...
  it('migración 43 crea bot_community_memberships con group_hash y sin group_id', () => {
    const database = new AppDatabase(':memory:');
    database.migrate();
    const columns = (
      // @ts-expect-error acceso para verificación de esquema interno
      database.db.prepare("PRAGMA table_info('bot_community_memberships')").all() as Array<{
        name: string;
      }>
    ).map((c) => c.name);

    expect(columns).toContain('group_hash');
    expect(columns).toContain('participant_hash');
