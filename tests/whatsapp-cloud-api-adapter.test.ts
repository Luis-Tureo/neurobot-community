...
describe('conector oficial WhatsApp Cloud API', () => {
  ...
  it('divide excepcionalmente textos largos sin cortar palabras y conserva el orden', async () => {
    const { adapter, requests } = subject();
    await adapter.initialize();
    const firstParagraph = 'a'.repeat(3000);
    const secondParagraph = 'b'.repeat(3000);

    await adapter.sendMessage('56912345678@c.us', `${firstParagraph}\n\n${secondParagraph}`);

    expect(requests).toHaveLength(2);
    const bodies = requests.map(
      (request) => JSON.parse(String(request.init?.body)) as { text: { body: string } },
    );
    expect(bodies.map((body) => body.text.body)).toEqual([firstParagraph, secondParagraph]);
    expect(bodies.every((body) => body.text.body.length <= 4096)).toBe(true);
  });
});
