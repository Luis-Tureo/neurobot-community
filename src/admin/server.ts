...
import { registerThematicDaysRoutes } from './thematic-days-routes.js';
...
export async function buildAdminServer(context: AdminServerContext) {
  ...
  try {
    ...
    registerThematicDaysRoutes(app, context);
    return app;
  } finally {
...
function installAIModelSelectionValidation(
  ...
  app.addHook('preValidation', async (request, reply) => {
    ...
    return reply.code(503).send({
      error:
        'No fue posible validar el modelo fijo de Groq. Conserva la configuración actual e intenta nuevamente.',
      code: 'AI_MODEL_CATALOG_UNAVAILABLE',
    });
  });
}
