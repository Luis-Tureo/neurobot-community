...
export class AppDatabase {
  ...
  public migrate(): void {
    ...
    const migrations = [
      ...
      },
      {
        ...
        sql: `
          CREATE TABLE ai_moderation_settings (
            ...
            admin_encrypted_phone TEXT,
            warning_template TEXT NOT NULL DEFAULT 'Hola {nombre}. Una persona administradora revisó un posible incumplimiento en {grupo} relacionado con {regla}. Motivo: {motivo}. Este aviso no implica una sanción automática; por favor revisa las reglas de convivencia.',
            min_severity TEXT NOT NULL DEFAULT 'MEDIO'
              CHECK(min_severity IN ('BAJO', 'MEDIO', 'ALTO', 'CRITICO')),
            dedup_window_minutes INTEGER NOT NULL DEFAULT 5
              CHECK(dedup_window_minutes BETWEEN 1 AND 1440),
            pending_expiry_hours INTEGER NOT NULL DEFAULT 24
              CHECK(pending_expiry_hours BETWEEN 1 AND 168),
            created_at TEXT NOT NULL,
            updated_at TEXT NOT NULL
          );

          CREATE TABLE ai_moderation_group_selection (
            assistant_id TEXT NOT NULL REFERENCES bots(id) ON DELETE CASCADE,
            group_hash TEXT NOT NULL,
            enabled INTEGER NOT NULL DEFAULT 1 CHECK(enabled IN (0, 1)),
            created_at TEXT NOT NULL,
          ...
          CREATE TABLE ai_moderation_incidents (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            assistant_id TEXT NOT NULL REFERENCES bots(id) ON DELETE CASCADE,
            group_hash TEXT NOT NULL,
            group_name TEXT,
            ...
            message_hash TEXT NOT NULL,
            admin_encrypted_participant_phone TEXT NOT NULL,
            detected_at TEXT NOT NULL,
            ...
            warning_snapshot TEXT NOT NULL,
            status TEXT NOT NULL DEFAULT 'pending'
              CHECK(status IN ('pending','approved','dismissed','warning_sent','warning_failed','expired')),
          ...
          CREATE INDEX idx_ai_mod_incidents_status
            ON ai_moderation_incidents(assistant_id, status, created_at DESC);
          CREATE INDEX idx_ai_mod_incidents_dedup
            ON ai_moderation_incidents(assistant_id, participant_hash, group_hash, created_at DESC);

          CREATE TABLE ai_moderation_metrics (
            assistant_id TEXT NOT NULL REFERENCES bots(id) ON DELETE CASCADE,
            local_date TEXT NOT NULL,
            messages_analyzed INTEGER NOT NULL DEFAULT 0,
            incidents_created INTEGER NOT NULL DEFAULT 0,
            incidents_approved INTEGER NOT NULL DEFAULT 0,
            incidents_dismissed INTEGER NOT NULL DEFAULT 0,
            warnings_sent INTEGER NOT NULL DEFAULT 0,
            warnings_failed INTEGER NOT NULL DEFAULT 0,
      ...
      },
      {
        version: 36,
        sql: `
          -- Migración definitiva a Groq. No toca ciphertext ni fingerprint de credenciales
          -- per_bot; solo normaliza el proveedor activo y conserva el historial anterior.
          ALTER TABLE ai_settings RENAME TO ai_settings_legacy_groq;
          CREATE TABLE ai_settings (
            profile_id INTEGER PRIMARY KEY REFERENCES assistant_profiles(id) ON DELETE CASCADE,
            enabled INTEGER NOT NULL DEFAULT 0 CHECK (enabled IN (0, 1)),
            provider TEXT NOT NULL DEFAULT 'groq' CHECK (provider IN ('groq', 'disabled')),
          ...
          SELECT profile_id, enabled,
            CASE WHEN provider = 'disabled' THEN 'disabled' ELSE 'groq' END,
            question_max_chars, context_max_tokens, input_max_tokens, response_max_tokens,
            response_max_chars, response_max_lines, temperature, user_hourly_limit,
            user_daily_limit, user_cooldown_seconds, group_hourly_limit, group_daily_limit,
            global_daily_limit, global_monthly_limit, global_daily_token_limit,
            global_monthly_token_limit, timeout_ms, updated_at, bot_id,
            interaction_hourly_limit, interaction_cooldown_seconds,
            duplicate_query_window_seconds,
            CASE WHEN provider = 'disabled' THEN NULL ELSE 'openai/gpt-oss-120b' END
          FROM ai_settings_legacy_groq;
          DROP TABLE ai_settings_legacy_groq;

          UPDATE provider_health SET provider = 'groq' WHERE provider = 'gemini';
          UPDATE assistant_ai_provider_health SET provider = 'groq' WHERE provider = 'gemini';

          -- La clave actual puede haber sido cifrada con el AAD histórico; se conserva intacta.
          UPDATE bot_ai_credentials
          SET display_name = 'Groq', updated_at = datetime('now')
          WHERE display_name = 'Gemini';

          ALTER TABLE bot_ai_provider_history RENAME TO bot_ai_provider_history_legacy_groq;
          CREATE TABLE bot_ai_provider_history (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            bot_id TEXT NOT NULL REFERENCES bots(id) ON DELETE CASCADE,
            ...
            created_at TEXT NOT NULL,
            display_name TEXT NOT NULL DEFAULT 'Groq'
          );
          INSERT INTO bot_ai_provider_history(id, bot_id, provider, action, created_at, display_name)
            SELECT id, bot_id, provider, action, created_at, display_name
            FROM bot_ai_provider_history_legacy_groq;
          DROP TABLE bot_ai_provider_history_legacy_groq;
          CREATE INDEX idx_bot_ai_provider_history
            ON bot_ai_provider_history(bot_id, created_at DESC, id DESC);
        `,
      },
      ...
      },
      {
        version: 44,
        sql: `
          -- Días temáticos recurrentes publicados como mensajes normales de WhatsApp.
          CREATE TABLE bot_themed_day_settings (
            bot_id TEXT NOT NULL REFERENCES bots(id) ON DELETE CASCADE,
          ...
          CREATE TABLE bot_themed_day_groups (
            bot_id TEXT NOT NULL REFERENCES bots(id) ON DELETE CASCADE,
    ...
    this.db
      .prepare(
        `UPDATE bots SET assistant_type='COMMUNITY_GROUPS',
           ...
           deletion_locked=0, group_channel_enabled=1, private_channel_enabled=0,
    ...
    this.db
      .prepare(
        `INSERT OR IGNORE INTO bot_ai_credentials(
         ...
         ) VALUES ('neurobot', 'global', NULL, NULL, ?, 'Groq')`,
      )
      .run(now);
  ...
  public releaseBotWhatsAppIdentity(botId: string): void {
    const now = new Date().toISOString();
    const release = this.db.transaction(() => {
      const connector = this.db
        .prepare(
          `UPDATE assistant_connectors SET normalized_phone_hash=NULL, whatsapp_identity_hash=NULL,
           session_ownership_verified=0, connector_status='UNLINKED', conflict_reason=NULL,
           linked_assistant_id=NULL, updated_at=? WHERE assistant_id=?
           AND id=(SELECT active_connector_id FROM bots WHERE id=?)`,
        )
        .run(now, botId, botId);
      const session = this.db
        .prepare(
          `UPDATE whatsapp_sessions SET status='disconnected', masked_number=NULL,
           last_connected_at=NULL, updated_at=? WHERE bot_id=?`,
        )
        .run(now, botId);
      this.db
        .prepare(`UPDATE bots SET lifecycle_status='UNLINKED', updated_at=? WHERE id=?`)
        .run(now, botId);
      if (connector.changes !== 1 || session.changes !== 1) {
        throw new Error('No fue posible liberar la identidad de WhatsApp.');
      }
    });
    release();
  }
  ...
  public getBotEncryptedCredential(botId: string): {
    ...
    const row = this.db
      .prepare(
        'SELECT credential_mode, encrypted_api_key, key_fingerprint, display_name FROM bot_ai_credentials WHERE bot_id = ?',
      )
      .get(botId) as
      | {
          credential_mode: 'global' | 'per_bot';
  ...
  public getBotAIModel(botId: string): string | null {
    ...
    return row?.model === GROQ_PREFERRED_MODEL ? GROQ_PREFERRED_MODEL : null;
  }
  ...
  public saveAISettings(settings: AISettings): AISettings {
    ...
    const now = new Date().toISOString();
    const modelToSave = settings.model === GROQ_PREFERRED_MODEL ? GROQ_PREFERRED_MODEL : null;
    const result = this.db
      .prepare(
  ...
  public recordCommunityMembership(
    ...
    this.db
      ...
      .run(botId, groupHash, participantHash, now, now);
  }
  ...
  public recordCommunityMembershipsBatch(
    ...
    const run = this.db.transaction(() => {
      for (const hash of participantHashes) {
        insertStmt.run(botId, groupHash, hash, now, now);
      }
    });
    run();
  }
  ...
  public removeCommunityMembership(
    ...
    this.db
      .prepare(
        'DELETE FROM bot_community_memberships WHERE bot_id = ? AND group_hash = ? AND participant_hash = ?',
      )
      .run(botId, groupHash, participantHash);
  }
  ...
  public reconcileCommunityGroupMemberships(
    ...
    const existingRows = this.db
      .prepare(
        'SELECT participant_hash FROM bot_community_memberships WHERE bot_id = ? AND group_hash = ?',
      )
      .all(botId, groupHash) as Array<{ participant_hash: string }>;
    ...
    const run = this.db.transaction(() => {
      ...
      for (const hash of existingSet) {
        if (!currentSet.has(hash)) {
          deleteStmt.run(botId, groupHash, hash);
          removed += 1;
        }
      }
    });
      ...
      this.db
        .prepare(
          `INSERT OR IGNORE INTO bot_community_memberships (
           ...
           SELECT bot_id, group_hash, ?, created_at, updated_at
  ...
  public getCommunityCountryAggregates(
    ...
    validGroupHashes?: string[],
  ): CommunityCountriesSummary {
    if (validGroupHashes !== undefined && validGroupHashes.length === 0) {
      return {
        totalParticipants: 0,
        identified: 0,
        unidentified: 0,
    ...
    const groupFilter =
      validGroupHashes && validGroupHashes.length > 0
        ? ` AND m.group_hash IN (${validGroupHashes.map(() => '?').join(', ')})`
        : '';
    const queryParams =
      validGroupHashes && validGroupHashes.length > 0
        ? [botId, ...validGroupHashes]
        : [botId];
  ...
  public listThemedDaySettings(botId: string): Array<{
    dayKey: string;
    enabled: boolean;
    startTime: string;
    title: string;
    ...
    return (
      this.db
        .prepare(
          `SELECT day_key, enabled, start_time, title, description, image_path
           FROM bot_themed_day_settings
           WHERE bot_id = ?
           ORDER BY day_key`,
        )
        .all(botId) as Array<{
  ...
  public saveThemedDaySettings(
    ...
    const statement = this.db.prepare(
      `INSERT INTO bot_themed_day_settings(
         bot_id, day_key, enabled, start_time, title, description, image_path, updated_at
       ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  ...
  public listThemedDayGroupKeys(botId: string): string[] {
    return (
      this.db
        .prepare(
          'SELECT group_key FROM bot_themed_day_groups WHERE bot_id = ? ORDER BY group_key',
        )
        .all(botId) as Array<{ group_key: string }>
    ).map((row) => row.group_key);
  }

  public saveThemedDayGroupKeys(botId: string, groupKeys: string[]): void {
    const now = new Date().toISOString();
    const insert = this.db.prepare(
      `INSERT INTO bot_themed_day_groups(bot_id, group_key, created_at)
       VALUES (?, ?, ?)`,
    );
    const run = this.db.transaction(() => {
      this.db.prepare('DELETE FROM bot_themed_day_groups WHERE bot_id = ?').run(botId);
  ...
  public claimThemedDayDelivery(
    ...
    const result = this.db
      .prepare(
        `INSERT OR IGNORE INTO bot_themed_day_deliveries(
  ...
  public completeThemedDayDelivery(input: {
    botId: string;
    dayKey: string;
    groupKey: string;
    localDate: string;
  ...
  public listThemedDayDeliveries(
    botId: string,
    limit = 50,
  ): Array<{
    dayKey: string;
...
function validateAISettings(settings: AISettings): void {
  ...
  if (settings.model !== null && settings.model !== undefined) {
    ...
    if (settings.model.trim() !== GROQ_PREFERRED_MODEL) {
      throw new Error('El modelo de IA no es válido.');
    }
  }
}
